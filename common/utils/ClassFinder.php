...
/**
 * ClassFinder copied from illuminate\filesystem
 * (Method findClasses was modified)
 *
 * @link https://github.com/illuminate/filesystem/blob/master/ClassFinder.php
 */
class ClassFinder
{
    /**
     ...
     *
     * $options['ignoreAbstract']   = (bool)    Ignore abstract classes (Default: false)
     *         ['mustBeA']          = (string)  Classes must be of type (Default: null)
     *
    ...
    public function findClasses($directory, array $options = [])
    {
        ...
        $classes = array_filter($classes, function ($class) use (&$options) {
            ...
            if ($options['mustBeA'] &&
                !is_subclass_of($class, $options['mustBeA'])
            ) {
                return false;
            }
